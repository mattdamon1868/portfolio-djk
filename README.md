...
This project required using hardware software and mechanical engineering skills in order to achieve the final design.  Three main parts to the project theory and research analysis, simulation building to test the theory, and building the physical robot for real world testing. For research analysis I reviewed many papers that are using ESC theory on nonholonomic robots and replicated past work in python to understand how it works. Eventually leading to the implementation of the theory into a simulation of the robot in Gazebo using ROS2. This enabled me to test the tuning parameters of the system to see what works with the speed constraints of the robot and whether the theory works. The ROS2 packages were designed as follows vehicle states and sensor frame state nodes, cost function update node, estimation of derivatives or filed node, controller node for commands, command the velocities and finally sensor feedback node. Each node is separated for organized and modularity of design in order to make it easier for other researchers to work on and update the package. Once implemented and tested whether the theory works now I have to incorporate the robot into the  I added an extra layer to the top of the robot due to its modularity and by designing the parts in ONshape (similar to solidworks) in order to then use a 3D printer to get the rotating arm which attaches to the servo motor atop the robot. with the arm we need to attach a sensor that would detect light which turns out to be a photoresistor diode which measures the change in resistance as the sensor detect light intensity. 
