...
- The masters project consists of experimenting with different control theories with robots and autonmous systems. Our lab was interested in extremum seeking control (ESC) which is an adaptive control approach widely research mainly to expand on the theory but the lab focussed more on the implementation of the methods on real robots, and testing in simulation.  The robot for my project was using a turtlebot3 from Robotis. This is a nonholnomic robot as it can only move in x-axis for forward velocity and angular velocity in the z-axis. It is a modular design which allowed me to make adjustments for the project, in my case an extra servo motor for the rotating sensor. The rotating sensor moves back and forth to get an estimation of the source in our case we used light and sound as some possible scalar signals. With this estimation the local exploration based on the servos motion calculates the derivative estimation of signal which perturbates the motion of the nonholonomic vehicle toward the source.  The signal can be based on arbitrary order derivatives with periodic or almost-periodic signals, these signals allow for the average derivative estimates to converge to the true derivative values for the cost functions at m-times differentiable. The robot has no knowledge of where the actual source is only that it is able to get a gradient estimate which should reveal the true derivative of the signal allowing the vehicle to converge. When I mean converge it essentially states that the vehicle is able to get within 0.5m from the source. 
This project required using hardware software and mechanical engineering skills in order to achieve the final design.  Three main parts to the project include theory and research analysis, environment simulation to test the theory, and building the physical robot for real world testing. For research analysis I reviewed many papers that are using ESC theory on nonholonomic robots and replicated past work in python to understand how it works. Eventually leading to the implementation of the theory into a simulation of the robot in Gazebo using ROS2. This enabled me to test and tune parameters of the system to see what works with the speed constraints of the robot and whether the theory works. The ROS2 packages were designed as follows vehicle states and sensor frame state nodes, cost function update node, estimation of derivatives or filter node, controller node for commands, velocities command node and finally sensor feedback node. Each node is separated for organization and modularity of design in order to make it easier for other researchers to work on and update the package. Once implemented and the theory tested in simulation and working correctly I begin the work on the robot to test the results from the simulation.  First the robot need to have an extra layer to the top of the robot and a rotating arm that can be attached to the servo and hold the sensors required for testing. Those designs were done by designing the parts in ONshape (similar to solidworks) in order to then use a 3D printer (Bambu, H2S) to get the rotating arm printed which attaches to the servo motor. The sensors that are on the robot for the two different sources mentioned earlier are microphone (measures the sound pressure level dBAs) for sound detection and a photoresistor (measures the resistance change) for light detection.  The robot comes with motors for the wheels and a raspberry pi4 for the brain of the robot. The software is a similar to design to the simulation but with some changes since we are using real acoustic source and real light source in simulation we use a virtual representation of both of these. The robot software is as follows vehicle states nodes, filter nodes, controller nodes, vehicle velocity nodes and the sensor feedback nodes (encoder (wheels), microphone and photoresistor node). Once built and software implmented using the tuning parameters from the Gazebo simulation we were able to get successful results with the robot being able to reach the source with both light and sound detection. The robot 
...
[![Watch the video](https://github.com/mattdamon1868/portfolio-djk/blob/main/images/tb3_thmb.png?s=200)](https://www.youtube.com/watch?v=K2YwE-z4ytc)
<br />
<br />
<br />
<br />
