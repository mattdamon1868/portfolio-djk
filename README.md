...
The robot used for my project was a TurtleBot3 from Robotis. It is a nonholonomic robot, capable of moving forward along the x-axis and rotating in the z-axis (angular velocity). Its modular design allowed me to make custom adjustments, such as adding an extra servo motor for a rotating sensor. This sensor moves back and forth to estimate the location of a signal source; in our case, we used light and sound as scalar signals. 
...
2) Optimization Path Planning project:
   ...
   -  This project involved formulating an optimization problem using a path planning algorithm for my robot. The project was not how the design of creating the robot but rather testing out the optimiazation techniques that would help to determine the best path for the robot based on different constraints for weight and energy use as a couple examples. Although it was initially designed for a second-order system, the focus was primarily on applying first-order techniques within optimization theory. The techniques used for the path planning for the robot utilized the duality principles and the simplex method within a linear programming framework. 
The objective function was designed to incorporate factors such as energy consumption, weight, distance to the source, and obstacle probability, each assigned arbitrary weightings to evaluate the most efficient cost value. The goal was to deepen understanding of linear programming techniques and work on optimization systems with certain constraints. 
The original nonlinear problem was simplified into a linear form to enable analysis using the simplex method and duality principle. The simplex method facilitates progression from one vertex of the feasible region to another, ensuring each step approaches the optimal solution. The duality principle involves deriving a dual problem which forms a Lagrangian function with multipliers from the primal or objective function, and solving the dual variables simultaneously.
The revised objective function seeks to minimize the cost function considering the dual variables, which are constrained to be nonnegative. Utilizing these two techniques, we obtained results that minimized the cost, with the primary aim of testing different methods rather than identifying the absolute optimal gain values, which was outside the scope of this project. The function represented as,
     \(J(x) = \alpha x_1 + \beta x_2 + \gamma x_3 + \delta x_4 \)
where we have \(x_1\) representing energy, \(x_2\) the weight of the robot, \(x_3\) the distance or error function which is the difference between primary path and the adjusted path each with a penalty gain, and finally, \(x_4\) indicating the likelihood of an obstacle present in the robot's path. Using this equation, we can apply the Gurobi optimizer library which uses the methods discussed to systematically identify the optimal path within the feasible region defined by the constraints, ultimately progressing toward an optimal solution.
   -  Skills: Optimiztion, duality, python, gurobi optimizer library

3) Deep Learning how bad weather affects the safety of autonomous driving.

   - Autonomous vehicles utilize various sensors to interpret their surroundings; however, there is an ongoing debate regarding the optimal number of sensors required to ensure safe passenger transport without human intervention. Achieving Level 5 autonomy appears feasible, but a key question is whether it can be accomplished with a minimal sensor array, thereby reducing manufacturing costs for autonomous vehicles. 
This research examined current research addressing the challenges faced by computer vision under adverse weather conditions or noisy environments. Specifically, we analyze the limitations of camera sensors in scenarios involving bad weather, which can compromise vehicle safety hence making an argument for why multiple sensors are necessary. All the papers researched used a similar design with Convolutional neural networks (CNNs) that integrated with recurrent neural networks (RNNs) to classify different types of adverse weather conditions, and these deep neural networks are employed to extract visual features from video frames. 
We review three contemporary approaches used in computer vision research related to weather detection, utilizing deep neural network algorithms such as YOLO, GRAMME, and ReViewNet. Each method employs different techniques to reduce image noise and eliminate false edge detections, aiding the vehicle in understanding its environment more reliably. The research encompasses various weather conditions including rain, snow, hail, and fog. Results indicate that snow and fog present significant challenges for camera systems, with object detection accuracy decreasing by over 20% across all methods tested. The overall average precision remained around 50%, which is less than ideal for autonomous vehicle operations. When visual data becomes obscured, it limits the algorithm's ability to react appropriately, as environmental perception is critical for safe navigation. 
Additionally, the study highlights that the effectiveness of these learning algorithms heavily depends on the datasets used. The variability in datasets across different research projects complicates direct comparisons and underscores the need for standardized benchmarks in this domain. The more data you have the better and more accurate the system could be however the research needs to improve so then can you the same data to have a better comparison between methods.
...
- The pet waste collection robot was designed to autonomously navigate a client's yard, identify pet waste, and efficiently collect and dispose of it. As a team of four senior students with limited experience in computer vision and autonomous navigation, this project presented significant challenges; however, it provided valuable opportunities to enhance our problem-solving skills and rapidly acquire new technical knowledge.  
Our primary focus was on developing the waste pickup mechanism and the computer vision algorithm for waste detection. We engineered an innovative pickup system using a 3D-printed "spike" puck that effectively punctures the waste and deposits it into an onboard waste container. The computer vision component utilized a convolutional neural network trained on approximately 10,000 images of pet waste, achieving an accuracy of approximately 89% in correct identification.
...
<a id="1">[1]</a> 
James-Kavanaugh D., McNamee P. (2026). 
Servos for Local Map Exploration Onboard Nonholonmic Vehicles for Extremum Seeking
published to IEEE Transactions on Control Systems Technology journal.
   
**Abstract:**

*Extremum seeking control (ESC) often employs perturbation-based estimates of derivatives for some sensor field or cost function. These estimates are generally obtained by simply multiplying the output of a single-unit sensor by some time-varying demodulation signals. Previous work has focused on sinusoidal perturbations to generate derivative estimates with results for arbitrary order derivatives of scalar maps or higher, up to third-order derivatives of multivariable maps. This work extends the perturbations from sinusoidal to bounded periodic or almost periodic functions and considers multivariable maps. A necessary and sufficient condition is given for determining if time-varying demodulation signals exist for estimating arbitrary order derivatives of multivariable maps for any given bounded periodic or almost periodic dither signal. These results are then used in a source seeking controller for a nonholonomic vehicle with a sensor actuated by a servo. The conducted simulation and real-world experiments demonstrate that by distributing the local map exploration to a servo, the nonholonomic vehicle was able to achieve a faster convergence to the source.*
