...
3) Autonomous systems CNNs how bad weather affects the safety of autonomous driving
   ...
   - This research paper focuses on enhancing detection and prediction capabilities in noisy environments through the application of deep learning and computer vision techniques. Autonomous vehicles utilize various sensors to interpret their surroundings; however, there is ongoing debate regarding the optimal number of sensors required to ensure safe passenger transport without human intervention. While achieving Level 5 autonomy appears feasible, a key question is whether it can be accomplished with a minimal sensor array, thereby reducing manufacturing costs for autonomous vehicles. 
This study examines current research addressing challenges faced by computer vision systems and cameras under adverse weather conditions. Specifically, we analyze the limitations of camera sensors in scenarios involving bad weather, which can compromise vehicle safety. Convolutional neural networks (CNNs) are integrated with recurrent neural networks (RNNs) to classify different types of adverse weather conditions, and these deep neural networks are employed to extract visual features from video frames. 
We review three contemporary approaches used in computer vision research related to weather detection, utilizing deep neural network algorithms such as YOLO, GRAMME, and ReViewNet. Each method employs different techniques to reduce image noise and eliminate false edge detections, aiding the vehicle in understanding its environment more reliably. The research encompasses various weather conditions, including rain, snow, hail, and fog. Results indicate that snow and fog present significant challenges for camera systems, with object detection accuracy decreasing by over 20% across all methods tested. Overall, the average precision remains around 50%, which is less than ideal for autonomous vehicle operations. When visual data becomes obscured, it hampers the algorithm's ability to react appropriately, as environmental perception is critical for safe navigation. 
...
- The pet waste collection robot was designed to autonomously navigate a customer's yard, identify pet waste, and efficiently collect and dispose of it. As a team of four senior students with limited experience in computer vision and autonomous navigation, this project presented significant challenges; however, it provided valuable opportunities to enhance our problem-solving skills and rapidly acquire new technical knowledge.  
